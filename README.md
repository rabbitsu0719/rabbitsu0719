<!-- ===================== -->
<!-- 🌤 Sky Blue Header -->
<!-- ===================== -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=87CEEB&height=220&section=header&text=Hello%20I'm%20Soohyun%20Kim%20☁️&fontSize=42&fontAlignY=38&fontColor=1F2A37" />

안녕하세요 김수현입니다  

🌿 **클라우드 환경에서 안정성과 확장성을 설계하는 백엔드·클라우드 엔지니어 지망생**  
Backend / API Engineer · FastAPI · Security-oriented  

<a href="mailto:rabbit-su@naver.com">📧 Email</a> · 
<a href="https://chivalrous-case-1ba.notion.site/2e17891c1f5c807b974ffd45201170dd">📘 Portfolio</a> · 
<a href="https://velog.io/@soohyun123">📝 Velog</a> · 
<a href="https://github.com/t-curity">🛡️ T:CURITY</a>

---

## 🎓 Education

| 기간 | 소속 | 내용 |
|---|---|---|
| 2018.03 ~ 2023.02 | 전남대학교 | 전자통신공학부 수료 |
| 2025.08 ~ 2026.02 | 스나이퍼팩토리 × 카카오클라우드 | 카카오클라우드로 배우는 AIaaS 마스터 클래스 2기 수료 |

---

## 💼 Experience

**스나이퍼팩토리 × 카카오클라우드 AIaaS 마스터 클래스 2기**  
교육생 (2025.08 ~ 2026.02)

- 카카오클라우드 기반 AI·클라우드 서비스 개발 실습  
- FastAPI 기반 백엔드 API 및 클라우드 아키텍처 설계  
- AI 모델 연동, 보안·인증(CAPTCHA) 서비스 팀 프로젝트 수행  
- GPU 서버 활용 ML 추론 및 서비스 배포 경험  

---

## 📁 Past Projects

### 🔐 T-CURITY : 2-Phase 행동·인지 기반 CAPTCHA 시스템 (2025)

고트래픽 환경에서 매크로·자동화 공격을 차단하기 위한  
**행동 + 인지 결합형 인증 시스템**

**Phase A (행동 기반 검증)**  
- OpenCV 기반 Ticket Slice 문제 동적 생성  
- 드래그 좌표 시계열 데이터 기반 30+ 행동 특징 추출  
- Isolation Forest 기반 이상 탐지  

**Phase B (인지 기반 검증)**  
- 3×3 이미지 그리드 인지 과제  
- EfficientNet + Random Forest 앙상블 검증  
- 동적 난이도 조절  

**보안·아키텍처 설계**  
- Server-Driven Flow 기반 세션 FSM  
- Blind Error Policy 적용  
- Base64 인라인 이미지 전송  
- In-Memory 세션 관리(TTL)  

**Tech Stack**  
Python · FastAPI · OpenCV · Isolation Forest · EfficientNet · Random Forest · ONNX Runtime · Kakao Cloud · Docker  

---

## 🎓 학부 프로젝트

### 🔥 스마트 화재 경보기 (2022.03 ~ 2022.07)
- 열화상 카메라·CO 센서 기반 화재 감지  
- 부저·LED 현장 경고 + 스마트폰 알림  
- 안정적인 HW–SW 연동  

**Tech Stack**  
Raspberry Pi · Python · OpenCV · IoT Sensors · REST API  

---

### 😷 스마트 마스크 보관함 (2021.09 ~ 2021.12)
- 얼굴 인식 기반 비접촉 마스크 제공  
- ResNet50 / MobileNetV2 사용  
- UVC 살균 안전 로직 포함  

**Tech Stack**  
Raspberry Pi · Python · OpenCV · TensorFlow · Deep Learning · IoT  

---

### 🚗 자율주행 자동차 제작 (2021.09 ~ 2021.12)
- 이미지–조향값 학습 기반 자율주행  
- 차체 3D 설계 및 출력  

**Tech Stack**  
Raspberry Pi · Python · TensorFlow · OpenCV · Fusion 360  

---

## 🛠 Tech Stack

**Backend / Cloud**  
Python · FastAPI · Flask · Docker · Linux  

**Cloud / Infrastructure**  
KakaoCloud · Nginx · GitHub Actions  

**Database / Data**  
PostgreSQL · SQL  

**AI / ML**  
TensorFlow · Scikit-learn · OpenCV  

---

## 📈 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=rabbitsu0719&show_icons=true&hide_title=true&hide_border=true&theme=transparent" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rabbitsu0719&layout=compact&hide_border=true&theme=transparent" height="160"/>
</div>

<!-- ===================== -->
<!-- 🌤 Sky Blue Footer -->
<!-- ===================== -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=87CEEB&height=120&section=footer" />
